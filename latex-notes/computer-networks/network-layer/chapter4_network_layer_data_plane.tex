...
The network layers primary role seems simple: to move packets, called \textbf{datagrams}, from a sending host to a receiving host. However, to accomplish this, the network layer must perform two distinct and critical functions: \textbf{forwarding} and \textbf{routing}.

\begin{enumerate}
    \item Forwarding
          \begin{itemize}
              \item Is a \emph{local} action.
              \item The process of taking a packet that has arrived on one of a routers input links and moving it to the appropriate output link.
          \end{itemize}
...
\begin{enumerate}
    \item Definition
          \begin{itemize}
              ...
              \item Forwarding is the primary function of the network layers \textbf{data plane}, which includes per-router operations performed on a packet as it moves through the router.
          \end{itemize}
    \item Mechanism
          \begin{itemize}
              \item A packet arrives at a routers input link.
              \item Router examines one or more fields in the packets header.
              \item Uses header values to index into its forwarding table.
              \item Value found in the forwarding table entry indicates the routers output link interface to which the packet should be forwarded.
          \end{itemize}
...
\subsubsection{3. Practical and Intuitive View: The Internets \emph{Actual} Service}
...
\begin{itemize}
    ...
    \item \textbf{Best-Effort Service:} Internets minimalist model with no delivery, order, delay, or bandwidth guarantees.
...
\begin{enumerate}
    ...
    \item Universal Implementation
          \begin{itemize}
              ...
              \item Its esential b/c routers must examine datagram headers to perform their forwarding function
          \end{itemize}
...
\begin{itemize}
    \item Performs per-touer function of forwarding datagrams from a routers input link to its appropriate output link
\end{itemize}
...
\section{4.2 Whats Inside a Router?}
...
\begin{itemize}
    ...
    \item Control Plane
          \begin{enumerate}
              \item Reason: Complexity and Timescale, control-plane functions (like new route using Dijkstras algo or commuinicating with SDN controller) are complex and happen on much slower timescale - milliseconds or even seconds
...
\begin{itemize}
    ...
    \item Destination-based Forwarding: Tell attendant find destination (i.e. Florida) and attendant looks it up, tells you take 3rd exit, and youre on the way
    \item Generalized Forwarding: Attendants decision is more complex
          \begin{enumerate}
              \item Youre from NY? Take the slow road. Youre from this state (license plate)? Take highway. Your car is not road-worthy? Youre blocked
          \end{enumerate}

    \item Bottlenecks: Analogy illustrates routers potential performance bottlenecks
          \begin{enumerate}
...
\begin{itemize}
    ...
    \item Destination-Based Forwarding and Longest Prefix Matching
          \begin{enumerate}
              ...
              \item Solution: Prefix matching: Routers dont store entries for individual hosts, store entries for prefixes (subnets)
          \end{enumerate}
          ...
          \begin{enumerate}
              \item How it works:
                    \begin{enumerate}
                        ...
                        \item CPU would perform lookup, find output port, and copy packet from memory to output ports buffer
                    \end{enumerate}
              \item Bottleneck
                    \begin{enumerate}
              ...
              \item How it works (modern)
                    \begin{enumerate}
                        ...
                        \item Instead, input ports hardware processor performs lookup and writes packet directory to output ports memory
                    \end{enumerate}
          \end{enumerate}
    ...
    \item Switching via an Interconnection Network (Crossbar)
          \begin{enumerate}
              \item How it works:
                    ...
                    \end{enumerate}
              \item When packet at input port A needs to go to output port Y, switch fabric controller closes crosspoint connecting As horizontal bus to Ys vertical bus
...
\begin{itemize}
    ...
    \item Priority Queueing
          \begin{enumerate}
              ...
              \item Preemption: typically non-preemptive, once a packet has started transmitting, its not interrupted
    ...
    \item Round Robin (RR) and Weighted Fair Queuing (WFQ)
          \begin{enumerate}
              ...
              \item Work conserving, if a classs queue is empty, scheduler doesnt wait, it just skips to next class
              \item Weighted Fair Queueing
                    \begin{enumerate}
                        ...
                        \item Service guarantee
                              \begin{enumerate}
                                  \item WFQ guarantees that each class i will receive a fractice of total bandwidth equal to: $R \cdot (w_i / \sigma(w_j))$, where sum is over all classes thatre currently active (have packets to send)
                              \end{enumerate}
                    \end{enumerate}
          \end{enumerate}
...
\subsection{Overview}
\begin{itemize}
    \item Delves into foundational protocols and addressing schemes of the Internet Protocol (IP) that enable the Internets network layer.
...
\begin{description}
    \item[Datagram:]
          \begin{enumerate}
              \item The Internets network-layer packet is known as a \textbf{datagram}.
              \item Fundamental to understanding IPs operations.
          ...
          \begin{enumerate}
              ...
              \item \textbf{Identifier, Flags, Fragmentation Offset (32 bits total):} Identifies fragments of the original datagram.
                    \begin{enumerate}
                        ...
                        \item If router needs to forward a datagram onto a link with an MTU smaller than datagrams size, it can fragment datagram into multiple smaller datagrams
              ...
              \item \textbf{Time-to-Live (TTL) (8 bits):} Limits the datagram’s lifetime to prevent infinite loops.
                    \begin{enumerate}
                        \item Critical safety mechanism to ensure datagrams dont circulate forever in network (e.g. in a routing loop)
              ...
              \item \textbf{Protocol (8 bits):} Specifies the transport protocol in the payload (e.g., TCP, UDP).
                    \begin{enumerate}
                        ...
                        \item Used only at final destination host to indicate which transport-layer protocol should receive datagrams payload
                              \begin{enumerate}
                                  \item 6 = TCP
                                  \item 17 = UDP
                    ...
                    \begin{enumerate}
                        ...
                        \item DHCP Request
                              \begin{enumerate}
                                  ...
                                  \item This tells any other servers that sent offers that they werent chosen
                              \end{enumerate}
                        \item DHCP ACK
                              \begin{enumerate}
          ...
          \begin{enumerate}
              ...
              \item Security:
                    \begin{enumerate}
                        ...
                        \item Incoming packet is dropped unless theres an existing entry in translation table
                    \end{enumerate}
    ...
    \item Cons
          \begin{enumerate}
              ...
              \item Breaks P2P and Servers
                    \begin{enumerate}
                        \item External users cant connect to you since no public, routable address
...
\begin{itemize}
    ...
    \item Generalized Forwarding
          \begin{enumerate}
              ...
              \item Flow Table
                    \begin{enumerate}
                        \item Generalizes simple forwarding table into a more complex flow table, flow table is program for packet switchs data plane
                    \end{enumerate}
              \item Connection to SDN
                    \begin{enumerate}
                        \item Generalized forwarding model is data-plane foundation of Software-Defined Networking
                        \item Because match and action rules can be complex and interdepedent, theyre not computed by switch itself
...
\begin{itemize}
    \item match plus action table is called flow table. Each entry in table has three key components
          ...
          \end{enumerate}
    \item If packet arrives and matches no entry in flow table, can either be dropped, or more commonly, forward to remote controller for a decision. Controller can decide what to do and potentially install a new flow rule for this packets flow
\end{itemize}
...
\begin{itemize}
    \item Action is a list of zero or more operations thatre applied to matched packet in sequence.
...
\begin{itemize}
    \item NAT Translation
          \begin{enumerate}
              ...
              \item Rewrite datagrams headers source/dest IP addresses and, transport-layers src/dest port numbers
          \end{enumerate}
...
\begin{itemize}
    \item NFVs goal is to unbundle function from dedicated hardware
    \item Mechanism
          \begin{enumerate}
